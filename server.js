...
const loadCustomerIdFromDB = (email) => {
  // Fake method - most likely you will have a real DB
  // where you should store the customerID  
  return IN_MEMORY_STORE[email]
}

const storeCustomerIdToDB = (email, customerId) => {
...
app.post("/attach_payment_method", async (req, res) => {
  try {
    const { email, paymentMethod } = req.body.options;
    
    // Step 1
    // Get or create the stripe customer    
    let customerId = loadCustomerIdFromDB(email)
    if (!customerId) {
      ...
      storeCustomerIdToDB(email, customerId)
    }
    
    // Step 2
    // Attach the payment method
    const attachedPM = await stripe.paymentMethods.attach(paymentMethod, {customer: customerId});
    console.log('attachedPM', attachedPM)
    ...
    res.json({stripeSubscription, attachedPM});
  } catch (err) {
    console.error('SetupIntent err', err)
    res.status(500).json(err)
  }
...
setInterval(() => { http.get(`http://${process.env.PROJECT_DOMAIN}.glitch.me/`) }, 2800);